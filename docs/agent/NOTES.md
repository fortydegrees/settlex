...
- `TimerManager` and `PufferBotManager` should not grow separate copies of stage-key or fallback logic.
- `server/runtimeConfig.js` owns game/lobby port parsing. Defaults remain game `8000` and lobby API `8080`; use `SETTLEX_GAME_SERVER_PORT` and `SETTLEX_LOBBY_API_PORT` for isolated live smoke runs when the normal ports are already occupied.

- Docs index note:
- `docs/INDEX.md` is generated by `scripts/write-docs-index.mjs` and maps every Markdown file under `docs/` to its first top-level heading.
- Run `pnpm docs:index` after adding, removing, or renaming durable docs so future agents can route to the exact doc without scanning the whole tree.
- Do not edit `docs/INDEX.md` by hand; edit the source doc heading or the generator instead.

- Catana underlay wave preview note:
- `/catana/dev/underlay-waves` is the dev-only preview surface for ambient island-underlay wave ideas.
- It renders the generated underlay geometry inline and keeps production `BoardUnderlay` plus `public/svgs/board_underlay_standard.svg` unchanged.
- Use the preview variants to compare incoming lap and slow drift before promoting any shoreline motion into the real board layer.
- Avoid outward expanding-surf motion as the default; it reads more like a glow/halo than water lapping the shore.
- Prefer inward or shore-contact motion: soft blue/white duplicate shoreline layers moving over the fixed surf band, with only a tiny optional touch over the sand edge.
- Keep wave overlays above the static underlay but below board tiles/ports so the visible SVG outline can move instead of only tinting a fixed silhouette.
- Start with transform/opacity GSAP wave layers; reserve MorphSVG/path morphing for a second pass only if simple layers feel too flat.
...
- homepage demo-board design spec: `docs/superpowers/specs/2026-06-04-homepage-demo-board-design.md`.
- homepage demo-board implementation plan: `docs/superpowers/plans/2026-06-04-homepage-demo-board.md`.
- homepage demo board implementation lives in `app/catana/homeDemo/`; it is a reusable dev/prototype module, not the production homepage route yet.
- the planned homepage board should use a curated static board preset plus an authored legal-ish four-colour event loop; avoid live board generation on homepage render.
- keep homepage demo committed pieces in demo-owned state (`roadsByEdgeId`, `buildingsByNodeId`) and render them through a board-only piece layer. Do not depend on shared placement-effect temporary DOM to persist final pieces.
- `HomeDemoEffectBridge` may use a minimal local `bgio-effects` provider shim for `GameEffects`, but the dev home-table route should not go back to the boardgame.io `Client`/sandbox board just to animate the title screen.
- the demo loop should emit existing `build:place` effects and use existing sound mappings; lasting visual state is committed shortly before the placement runner cleanup.
- the current `ready` home-table prototype uses a presentation-only attract loop over the real board geometry: scripted road/settlement placement and settlement-to-city upgrade.
- the loop should use the existing Catana `GameEffects` / `createPiecePlacementRunner` / build cue stack for motion and sound; avoid parallel CSS-only piece animation because it reads pasted-on beside the real board.
- the shared placement runner removes its temporary GSAP DOM on completion, which is correct for games. Homepage/demo loops need their own prototype-only static result layer, revealed slightly before cleanup, so pieces do not flash or disappear between animation and final state.
- keep homepage board life non-interactive by default. It should read like a title-screen/demo loop, not a tile/node/edge editor or a real match control surface.
- avoid piling up many temporary pieces at once; one construction beat and one upgrade beat felt closer to "alive" without making the board too busy.
