...
| Account & Identity | Public profile | `app/u/[username]/page-content.js` | `/u/[username]` | Empty recent matches; populated match history; missing profile | Server page view | Replay link navigation | Both | Planned | Extract view model |
...
| Alerts & Recovery | Match availability and interrupted-duel recovery | `app/g/[matchID]/UnavailableMatchPage.jsx`, `app/catana/lobby/[matchID]/MatchPageClient.js` | Unavailable game and interrupted duel routes | Missing/cancelled/expired game; interrupted duel; recovery pending; recovery error | Route-local components and match helpers | Return; retry/look again; pending feedback | Both | Planned | Extract interrupted-duel view |
...
| Postgame & Replay | Replay status | `app/replays/components/ReplayStatusPage.jsx` | Replay loading/error | Preparing/polling; active; unavailable; invalid; retry | Component | Status change; retry/navigation | Both | Planned | None |
