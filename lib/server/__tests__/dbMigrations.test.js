...
describe("db migrations", () => {
  ...
  it("ships the match-alert migration", () => {
    ...
    const reservationSql = readRepoFile(
      "lib/server/db/sql/0006_match_alert_pause_reservations.sql"
    ).toLowerCase();
    const leaseSql = readRepoFile(
      "lib/server/db/sql/0007_match_alert_pause_reservation_leases.sql"
    ).toLowerCase();
    ...
    expect(reservationSql).toContain("add column if not exists pause_reservation_id");
    expect(leaseSql).toContain("add column if not exists pause_reservation_count");
    expect(leaseSql).toContain("pause_reservation_previous_reason");
  });

  it("ships the public matchmaking cancellation tombstone migration", () => {
    const sql = readRepoFile(
      "lib/server/db/sql/0008_public_matchmaking_cancellations.sql"
    ).toLowerCase();

    expect(sql).toContain(
      "create table if not exists public_matchmaking_cancellations"
    );
    expect(sql).toContain("primary key (account_id, mode_id, request_id)");
  ...
  it("applies the Better Auth repair migration when old account migrations are already recorded", async () => {
    ...
    expect(result.appliedMigrations).toEqual([
      ...
      "0005_archived_board_provenance.sql",
      "0005_match_alerts.sql",
      "0006_match_alert_pause_reservations.sql",
      "0007_match_alert_pause_reservation_leases.sql",
      "0008_public_matchmaking_cancellations.sql",
    ]);
    expect(
