...
describe("waiting-duel verification", () => {
  ...
  it("treats an ownerless live table as not eligible", () => {
    expect(
      validateWaitingDuel({
        liveMatch: {
          ...validWaitingMatch,
          players: { 0: { id: 0, name: "" }, 1: { id: 1, name: "" } },
        },
        matchID: "match_1",
        seekerAccountId: "acct_zak",
      })
...
describe("waiting-duel announcement orchestration", () => {
  ...
  it("does not claim a legitimately filled table", async () => {
    const claim = vi.fn();
    await expect(
      announceWaitingDuel({
        matchID: "match_1",
    ...
    ).resolves.toEqual({ announced: false, reason: "filled" });
    expect(claim).not.toHaveBeenCalled();
  });

  it("does not claim an ownerless live table", async () => {
    const claim = vi.fn();
    await expect(
      announceWaitingDuel({
        matchID: "match_1",
...
describe("Web Push delivery", () => {
  ...
  it("does not send to literal private or loopback endpoints", async () => {
    const sendNotification = vi.fn();
    ...
    await expect(
      sendMatchAlert({
        matchID: "match_1",
        subscriptions: [
          { ...subscriptionA, endpoint: "https://127.0.0.1/internal" },
          { ...subscriptionB, endpoint: "https://[::1]/internal" },
        ],
        webPush: { sendNotification },
        config: {
          configured: true,
        ...
        deleteExpiredImpl: deleteInvalid,
        recordDeliveryImpl: vi.fn(),
      })
    ).resolves.toEqual({ attempted: 2, delivered: 0, expired: 0, failed: 2 });
    expect(sendNotification).not.toHaveBeenCalled();
    expect(deleteInvalid).toHaveBeenCalledWith({
      endpoints: ["https://127.0.0.1/internal", "https://[::1]/internal"],
    });
  });

  it("blocks a hostname when connection-time DNS resolves it privately", async () => {
    let lookupError = null;
    const sendNotification = vi.fn((_subscription, _payload, options) =>
    ...
    await sendMatchAlert({
      ...
      subscriptions: [subscriptionA],
      webPush: { sendNotification },
      dnsLookupImpl: (_hostname, _options, callback) =>
    ...
    expect(lookupError).toMatchObject({ code: "ERR_PRIVATE_PUSH_ENDPOINT" });
  });

  it("uses a delivery timeout and bounded fanout concurrency", async () => {
    let active = 0;
    let maxActive = 0;
    const seenTimeouts = [];
    const sendNotification = vi.fn(async (_subscription, _payload, options) => {
      active += 1;
    ...
    ).resolves.toEqual({ attempted: 12, delivered: 12, expired: 0, failed: 0 });

    expect(maxActive).toBeLessThanOrEqual(4);
    expect(seenTimeouts).toEqual(Array(12).fill(10_000));
  });
});
