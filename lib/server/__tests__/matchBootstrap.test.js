import fs from "node:fs";
import path from "node:path";
import { fileURLToPath, pathToFileURL } from "node:url";
import { afterEach, describe, expect, it, vi } from "vitest";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const repoRoot = path.resolve(__dirname, "..", "..", "..");
const matchesRoot = path.join(repoRoot, "lib", "server", "matches");

const modulePath = (filename) => path.join(matchesRoot, filename);
const ORIGINAL_NODE_ENV = process.env.NODE_ENV;
const ORIGINAL_GAME_SERVER_INTERNAL_URL = process.env.GAME_SERVER_INTERNAL_URL;

const loadModule = async (filename) => {
  const targetPath = modulePath(filename);
  expect(fs.existsSync(targetPath)).toBe(true);
  return import(`${pathToFileURL(targetPath).href}?t=${Date.now()}`);
};

const jsonResponse = (body, status = 200) =>
  new Response(JSON.stringify(body), {
    status,
    headers: {
      "Content-Type": "application/json",
    },
  });

afterEach(() => {
  vi.resetModules();
  process.env.NODE_ENV = ORIGINAL_NODE_ENV;
  if (ORIGINAL_GAME_SERVER_INTERNAL_URL == null) {
    delete process.env.GAME_SERVER_INTERNAL_URL;
  } else {
    process.env.GAME_SERVER_INTERNAL_URL = ORIGINAL_GAME_SERVER_INTERNAL_URL;
  }
});

describe("match bootstrap wrappers", () => {
  it("creates a private bot-intent match and seats Puffer before returning it", async () => {
    const { createBotMatchForAccount } = await loadModule("createMatchForAccount.js");
    expect(createBotMatchForAccount).toBeTypeOf("function");
    const account = {
      id: "acct_1",
      currentUsername: "Ada",
      avatarEmoji: "🤠",
      avatarColor: "sky",
    };
    const created = {
      matchID: "bot_match_1",
      playerID: "0",
      playerCredentials: "human_secret",
    };
    const createMatchForAccountImpl = vi.fn().mockResolvedValue(created);
    const joinMatchForAccountImpl = vi.fn().mockResolvedValue({
      playerID: "1",
      playerCredentials: "bot_secret",
    });

    await expect(
      createBotMatchForAccount({
        account,
        numPlayers: 2,
        setupData: { modeId: "duel" },
        createMatchForAccountImpl,
        joinMatchForAccountImpl,
      })
    ).resolves.toEqual(created);

    expect(createMatchForAccountImpl).toHaveBeenCalledWith({
      account,
      numPlayers: 2,
      setupData: { modeId: "duel", matchKind: "bot_game" },
    });
    expect(joinMatchForAccountImpl).toHaveBeenCalledWith({
      account,
      matchID: "bot_match_1",
      playerID: "1",
      participant: {
        participantType: "bot",
        botKey: "puffer",
        usernameSnapshot: "Puffer 2",
        avatarSnapshot: { emoji: "🤖", color: "royal" },
      },
    });
  });

  it("creates an isolated native-board match for SettleGraph 005", async () => {
    const { createBotMatchForAccount } = await loadModule("createMatchForAccount.js");
    const account = { id: "acct_1", currentUsername: "Ada" };
    const created = {
      matchID: "v2_match_1",
      playerID: "0",
      playerCredentials: "human_secret",
    };
    const createMatchForAccountImpl = vi.fn().mockResolvedValue(created);
    const joinMatchForAccountImpl = vi.fn().mockResolvedValue({ playerID: "1" });

    await expect(createBotMatchForAccount({
      account,
      botKey: "settlegraph-v2",
      numPlayers: 2,
      setupData: {
        modeId: "duel",
        rulesetId: "duel",
        boardSourceId: "duel-fair-official-v1",
      },
      createMatchForAccountImpl,
      joinMatchForAccountImpl,
    })).resolves.toEqual(created);

    expect(createMatchForAccountImpl).toHaveBeenCalledWith({
      account,
      numPlayers: 2,
      setupData: {
        modeId: "duel",
        rulesetId: "duel",
        boardSourceId: "settlegraph-v2-native-v1",
        matchKind: "bot_game",
        botKey: "settlegraph-v2",
      },
    });
    expect(joinMatchForAccountImpl).toHaveBeenCalledWith({
      account,
      matchID: "v2_match_1",
      playerID: "1",
      participant: {
        participantType: "bot",
        botKey: "settlegraph-v2",
        usernameSnapshot: "SettleGraph 005",
        avatarSnapshot: { emoji: "🤖", color: "royal" },
      },
    });
  });

  it("cleans up the private human seat when Puffer setup fails", async () => {
    const { createBotMatchForAccount } = await loadModule("createMatchForAccount.js");
    expect(createBotMatchForAccount).toBeTypeOf("function");
    const account = { id: "acct_1", currentUsername: "Ada" };
    const leaveMatchForAccountImpl = vi.fn().mockResolvedValue({ left: true });

    await expect(
      createBotMatchForAccount({
        account,
        createMatchForAccountImpl: vi.fn().mockResolvedValue({
          matchID: "bot_match_1",
          playerID: "0",
          playerCredentials: "human_secret",
        }),
        joinMatchForAccountImpl: vi
          .fn()
          .mockRejectedValue(new Error("Puffer join failed")),
        leaveMatchForAccountImpl,
      })
    ).rejects.toThrow("Puffer join failed");

    expect(leaveMatchForAccountImpl).toHaveBeenCalledWith({
      account,
      matchID: "bot_match_1",
      playerID: "0",
      credentials: "human_secret",
    });
  });

  it("creates a bgio match and joins seat 0 using the current account snapshot", async () => {
    const { createMatchForAccount } = await loadModule("createMatchForAccount.js");
    const fetchImpl = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse({ matchID: "match_1" }))
      .mockResolvedValueOnce(
        jsonResponse({
          playerID: "0",
          playerCredentials: "secret_123",
        })
      )
      .mockResolvedValueOnce(
        jsonResponse({
          matchID: "match_1",
          players: [
            {
              id: 0,
              name: "Ada",
              data: {
                participantType: "human",
                accountId: "acct_1",
                usernameSnapshot: "Ada",
                avatarSnapshot: {
                  emoji: "🤠",
                  color: "sky",
                },
              },
            },
          ],
        })
      );

    const result = await createMatchForAccount({
      fetchImpl,
      baseUrl: "http://game:8080",
      account: {
        id: "acct_1",
        currentUsername: "Ada",
        avatarEmoji: "🤠",
        avatarColor: "sky",
      },
      numPlayers: 2,
      matchmakingRequestId: "r".repeat(48),
      requestedCredentials: "c".repeat(48),
    });

    expect(result.matchID).toBe("match_1");
    expect(result.playerID).toBe("0");
    expect(result.playerCredentials).toBe("secret_123");
    expect(result.match.players[0].data).toMatchObject({
      participantType: "human",
      accountId: "acct_1",
      usernameSnapshot: "Ada",
    });

    const joinRequest = fetchImpl.mock.calls[1];
    expect(joinRequest[0]).toBe("http://game:8080/games/catan/match_1/join");
    expect(JSON.parse(joinRequest[1].body)).toMatchObject({
      playerID: "0",
      playerName: "Ada",
      requestedCredentials: "c".repeat(48),
      data: {
        participantType: "human",
        accountId: "acct_1",
        matchmakingRequestId: "r".repeat(48),
        usernameSnapshot: "Ada",
        avatarSnapshot: {
          emoji: "🤠",
          color: "sky",
        },
      },
    });
  });

  it("creates a bgio match and joins the requested creator seat", async () => {
    const { createMatchForAccount } = await loadModule("createMatchForAccount.js");
    const fetchImpl = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse({ matchID: "match_friend_1" }))
      .mockResolvedValueOnce(
        jsonResponse({
          playerID: "1",
          playerCredentials: "secret_friend_1",
        })
      )
      .mockResolvedValueOnce(
        jsonResponse({
          matchID: "match_friend_1",
          players: [
            null,
            {
              id: 1,
              name: "Ada",
              data: {
                participantType: "human",
                accountId: "acct_1",
                usernameSnapshot: "Ada",
                avatarSnapshot: {
                  emoji: "🤠",
                  color: "sky",
                },
              },
            },
          ],
        })
      );

    const result = await createMatchForAccount({
      fetchImpl,
      baseUrl: "http://game:8080",
      account: {
        id: "acct_1",
        currentUsername: "Ada",
        avatarEmoji: "🤠",
        avatarColor: "sky",
      },
      numPlayers: 2,
      creatorSeatId: "1",
      setupData: {
        matchKind: "friend_challenge",
      },
    });

    expect(result.matchID).toBe("match_friend_1");
    expect(result.playerID).toBe("1");
    expect(result.playerCredentials).toBe("secret_friend_1");

    const createRequest = fetchImpl.mock.calls[0];
    expect(JSON.parse(createRequest[1].body)).toMatchObject({
      numPlayers: 2,
      setupData: {
        matchKind: "friend_challenge",
      },
    });

    const joinRequest = fetchImpl.mock.calls[1];
    expect(joinRequest[0]).toBe("http://game:8080/games/catan/match_friend_1/join");
    expect(JSON.parse(joinRequest[1].body)).toMatchObject({
      playerID: "1",
      playerName: "Ada",
    });
  });

  it("joins a bot seat with a bot participant snapshot", async () => {
    const { joinMatchForAccount } = await loadModule("joinMatchForAccount.js");
    const fetchImpl = vi.fn().mockResolvedValue(
      jsonResponse({
        playerID: "1",
        playerCredentials: "bot_secret",
      })
    );

    const result = await joinMatchForAccount({
      fetchImpl,
      baseUrl: "http://game:8080",
      matchID: "match_1",
      playerID: "1",
      participant: {
        participantType: "bot",
        botKey: "puffer",
        usernameSnapshot: "[BOT] 2",
        avatarSnapshot: {
          emoji: "🤖",
          color: "sky",
        },
      },
    });

    expect(result.playerCredentials).toBe("bot_secret");
    expect(JSON.parse(fetchImpl.mock.calls[0][1].body)).toMatchObject({
      playerID: "1",
      playerName: "[BOT] 2",
      data: {
        participantType: "bot",
        botKey: "puffer",
        usernameSnapshot: "[BOT] 2",
        isBot: true,
        bot: "puffer",
      },
    });
  });

  it("falls back to the local lobby API URL in development when no internal URL is configured", async () => {
    delete process.env.GAME_SERVER_INTERNAL_URL;
    process.env.NODE_ENV = "development";

    const { createMatchForAccount } = await loadModule("createMatchForAccount.js");
    const fetchImpl = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse({ matchID: "match_dev_1" }))
      .mockResolvedValueOnce(
        jsonResponse({
          playerID: "0",
          playerCredentials: "secret_dev",
        })
      )
      .mockResolvedValueOnce(
        jsonResponse({
          matchID: "match_dev_1",
          players: [],
        })
      );

    await createMatchForAccount({
      fetchImpl,
      account: {
        id: "acct_1",
        currentUsername: "Ada",
        avatarEmoji: "🤠",
        avatarColor: "sky",
      },
      numPlayers: 2,
    });

    expect(fetchImpl.mock.calls[0][0]).toBe("http://localhost:8080/games/catan/create");
  });
});
