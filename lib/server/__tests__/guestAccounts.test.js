...
describe("guest account services", () => {
  it("rejects username characters outside ASCII letters, numbers, and underscores", async () => {
    const { normalizeUsername } = await loadModule("normalizeUsername.js");

    for (const username of ["Renée", "David Smith", "david-smith", "Player🎲"]) {
      expect(() => normalizeUsername(username)).toThrow(
  ...
  it("creates a guest profile and username history row for a Better Auth user", async () => {
    ...
    expect(result.account.currentUsername).toBe("Ada_Lovelace");
  ...
  it("updates a guest account username and rolls username history forward", async () => {
    ...
    expect(state.usernameHistory[1].username).toBe("Ada_Lovelace");
  });
});
