...
describe("match alert store", () => {
  ...
  it("reserves a human-game pause transactionally and captures prior state", async () => {
    ...
    expect(client.query).toHaveBeenNthCalledWith(
      3,
      expect.stringMatching(/paused_match_id = \$2[\s\S]*enabled = true/i),
      [["acct_1", "acct_2"], "match_2"]
    );
  ...
  it("restores only a failed join's still-owned pause", async () => {
    ...
    expect(client.query).toHaveBeenNthCalledWith(
      2,
      expect.stringMatching(/paused_match_id = \$5/i),
      ["acct_1", null, null, null, "failed_match"]
    );
    expect(client.query).toHaveBeenNthCalledWith(
      3,
      expect.stringMatching(/paused_match_id = \$5/i),
      ["acct_2", "human_game", "older_match", pausedAt, "failed_match"]
    );
    expect(client.query).toHaveBeenNthCalledWith(4, "COMMIT");
    expect(client.release).toHaveBeenCalledOnce();
  });
