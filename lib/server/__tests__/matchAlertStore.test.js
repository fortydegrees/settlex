...
describe("match alert store", () => {
  ...
  it("reserves a human-game pause transactionally and captures prior state", async () => {
    ...
    await expect(
      ...
      previousPreferences: previousRows,
    });
    ...
    expect(client.query).toHaveBeenNthCalledWith(
      3,
      expect.stringMatching(
        /paused_match_id = \$2[\s\S]*pause_reservation_id = \$3[\s\S]*enabled = true/i
      ),
      [
        ["acct_1", "acct_2"],
        "match_2",
  ...
  it("restores only a failed join's still-owned pause", async () => {
    ...
    const pausedAt = new Date("2026-07-14T10:00:00Z");
    ...
    expect(client.query).toHaveBeenNthCalledWith(
      2,
      expect.stringMatching(
        /pause_reservation_id = \$5[\s\S]*paused_match_id = \$6[\s\S]*pause_reservation_id = \$7/i
      ),
      [
        "acct_1",
        null,
  ...
  it("finalizes only the exact still-current pause reservation", async () => {
    ...
    expect(pool.query).toHaveBeenCalledWith(
      expect.stringMatching(
        /pause_reservation_id = null[\s\S]*paused_match_id = \$2[\s\S]*pause_reservation_id = \$3/i
      ),
      [
        ["acct_1"],
        "match_1",
