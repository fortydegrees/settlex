...
export const createFakeAccountsPool = () => {
    ...
    query: vi.fn(async (sql, params = []) => {
      ...
      if (normalized.startsWith("insert into accounts")) {
        const account = {
          ...
          status: params[1] ?? "guest",
          currentUsername: params[2],
          avatarEmoji: params[3],
          avatarColor: params[4],
          createdAt: new Date().toISOString(),
          claimedAt: params[1] === "claimed" ? new Date().toISOString() : null,
  ...
  const pool = {
    ...
    query: vi.fn(async (...args) => client.query(...args)),
  };
