...
const createSession = () => {
  ...
  const cookieValue = `${selector}.${token}`;
...
export const createGuestAccount = async ({
  ...
  try {
    ...
    const createdAccount = await client.query(
      ...
      `,
      [accountId, normalizedUsername, normalizedAvatarEmoji, normalizedAvatarColor]
    );
    ...
    await client.query(
      `
        ...
        VALUES ($1, $2, $3, $4)
      `,
      [session.selector, accountId, session.tokenHash, session.expiresAt]
    );
