...
export async function reserveMatchAlertsForAccounts({
  ...
  try {
    ...
    return {
      matchID,
      reservationId: pausedAccountIds.length > 0 ? reservationId : null,
      pausedAccountIds,
...
export async function restoreMatchAlertsReservation({
  ...
  if (!matchID || !reservationId || previous.length === 0) return [];
      ...
      const result = await client.query(
        `UPDATE match_alert_preferences
            SET paused_reason = $2,
                paused_match_id = $3,
                paused_at = $4,
                pause_reservation_id = $5,
                updated_at = NOW()
          WHERE account_id = $1
            AND enabled = TRUE
            AND paused_match_id = $6
            AND pause_reservation_id = $7
          RETURNING account_id AS "accountId"`,
        [
          ...
          preference.pauseReservationId ?? null,
          matchID,
          reservationId,
        ]
      );
...
export async function finalizeMatchAlertsReservation({
  ...
  const matchID = reservation?.matchID;
  const reservationId = reservation?.reservationId;
  const accountIds = [...new Set(reservation?.pausedAccountIds ?? [])];
  if (!matchID || !reservationId || accountIds.length === 0) return [];
  ...
  const { rows } = await pool.query(
    `UPDATE match_alert_preferences
      ...
      RETURNING account_id AS "accountId"`,
    [accountIds, matchID, reservationId]
  );
  return rows.map((row) => row.accountId);
}
