...
const INCUMBENT_006_CTNN_SHA256: &str =
    "299c23241e1ca32c4b9203206a9b2c17fc7f6246d4adff0f6d3a9ad6404b736b";

fn model_path() -> String {
    std::env::var("SETTLEX_INCUMBENT_006_MODEL").expect("set the sealed 006 model path")
}

#[test]
#[ignore = "requires external sealed 006 artifact and authenticated reference probes"]
fn incumbent_006_matches_research_outputs_and_legal_choices() {
    let model = load_verified_model(Path::new(&model_path())).expect("load checkpoint 006");
    assert_eq!(model.sha256, INCUMBENT_006_CTNN_SHA256);
    ...
    let probes: Value = serde_json::from_slice(
        &std::fs::read(std::env::var("SETTLEX_006_REFERENCE_PROBES").unwrap()).unwrap(),
    )
    .unwrap();
