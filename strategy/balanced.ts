import { Board, ResourceType, Hex, getNumDots, STANDARD_RESOURCES} from '../spec/board';
...
import { findAllLowestBy, hasAll, sumByKey, findHighestBy, countMatches, findAllHighestBy } from '../utils/collections'
 ...
 */
const SCORE_RANGES = {
  'Standard': { greedy: 13.014067729766802, fair: 5.893507115912209 },
...
export class BalancedStrategy implements Strategy {
  ...
  generateBoard(spec): { board: Board, score: number } {
  ...
  private generateSingleBoard(spec): Board {
   ...
   * lowest valued hex.
   */
  private placeNumber(numberStrategy: NumberPlacement, resourceType=null) {
  ...
  private scoreHexesAndCorners(balanceCoastAndDesert = false) {
    ...
    for (const corner of this.board.corners) {
      ...
      const addCombo = (name: string, multiplier: number, ...resources) => {
  ...
  private computeRollOddsPerResource(hexes: Hex[], defaultRollScore: number) {
    const map = new Map();
  ...
  private getResourceValue(resource): number {
    assert(resource);
    switch (resource) {
   ...
   * and returns that.
   */
  private chooseDesertPlacement(spec): DesertPlacement {
  ...
  private calculateTargetScore(spec): number {
