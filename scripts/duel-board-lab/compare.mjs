...
function renderComparisonHtml(comparison) {
  const rows = Object.entries(comparison).map(([family, result]) => {
    ...
    return "<tr>"
      ...
      + `<td>${escapeHtml(`${(result.passRate * 100).toFixed(2)}%`)}</td>`
      + `<td>${escapeHtml(formatMetric(result.score.min))}</td>`
      + `<td>${escapeHtml(formatMetric(result.score.mean))}</td>`
  ...
  return "<!doctype html>"
    ...
    + '<table><thead><tr><th scope="col">Family</th><th scope="col">Total</th><th scope="col">Pass rate</th>'
    + '<th scope="col">Min score</th><th scope="col">Mean score</th><th scope="col">Max score</th><th scope="col">Peak RSS MiB</th>'
