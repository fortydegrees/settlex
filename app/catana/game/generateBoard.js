...
const generateStandardHexes = (shape, radius) => {
  ...
  const doNumbersAndResources = (tiles, spec) => {
    ...
    for (const {tile} of tiles) {
      if (resources.length) {
        const excluded = [];
        if (!spec.isResourceAllowed(tile, ResourceType.GOLD)) {
          excluded.push(ResourceType.GOLD);
        }
        if (!spec.isResourceAllowed(tile, ResourceType.DESERT)) {
          excluded.push(ResourceType.DESERT);
        }
        tile.resource = resources.popExcluding(...excluded);
      }
      if (tile.resource !== ResourceType.DESERT && !rollNumbers.isEmpty()) {
        tile.number = rollNumbers.pop();
      }
    }
  ...
  const getTileByCoord = (coordinate) =>{
    return tiles.find(tile => JSON.stringify(tile.coordinate) === JSON.stringify(coordinate)) || null;
  }
...
export const generateBoard = (spec) => {
