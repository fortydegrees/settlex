...
function createRandomQueueByPercent(percent, size, lowValue, highValue) {
...
export class BalancedBoard {
  ...
  scoreBoard() {
    ...
    const scores = Object.values(this.board.nodes).map((c) => c.score);
    ...
    const highestCorner = findHighestBy(
      Object.values(this.board.nodes),
      (c) => c.score
    ).score;
  ...
  generateSingleBoard(spec) {
    const board = new Board(spec, true);
  ...
  scoreHexesAndCorners(balanceCoastAndDesert = false) {
    ...
    for (const hex of this.board.tiles) {
      const tileNodes = Object.values(hex.tile.nodes);
      const realNodes = [];
      for (const id of tileNodes) {
        realNodes.push(this.board.nodes[id]);
      }
